...
function UserProfile() {
    ...
    var urlString = "http://localhost:8080/api/v1/user/" + id;
        ...
        fetch(urlString)
            .then(response => response.json().then(function(json) {
                if(json != null) {
                    ...
                    document.getElementById('nonexistent-user-text-field').innerHTML = id + " does not exist";
    ...
    return (
        <>
            ...
            <div id="user-info-div" style={{float:"left", display:"inline", width: "30%"}}>
                {isValidProfile ? <UserInfo postId={id} /> : <></>}
            </div>

            <div id="question-list-div" style={{float:"left", display:"inline", width: "50%"}}>
                {isValidProfile ? <QuestionsList postId={id} /> : <></>}
            </div>
